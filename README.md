...
`0.1.1` 将常驻战斗引擎与可选 Vulkan 驱动彻底拆分，自动模式默认通过稳定的 OpenGL ES 路径启动，并保留 Kotlin 模拟与 Canvas 最终安全回退。画面、性能、操作与声音选项统一收进大厅右上角的设置菜单。
...
- 稳定自动模式、手动 Vulkan、手动 OpenGL ES；Vulkan 故障不会阻止战斗逻辑加载。
